...
async function processCandidateWithAi(args) {
  ...
  try {
    ...
    const decision = validateReplyDecision(result.data || result.output_parsed, candidate, selected, identity);
...
function validateReplyDecision(decision, candidate, selected, identity) {
  ...
  const selectionIds = new Set([
    ...
    ...parsed.selectedOpinionIds,
    ...parsed.selectedWriterInstructionIds,
  ]);
  if (selectionIds.size !== (
    parsed.selectedProjectIds.length
    + parsed.selectedExperienceIds.length
    + parsed.selectedOpinionIds.length
    + parsed.selectedWriterInstructionIds.length
  )) {
    throw Object.assign(new Error("AI_CONTEXT_INVALID"), { code: "AI_CONTEXT_INVALID" });
  }
  const selectedExperienceIds = new Set((selected.selectedExperiences || []).map((item) => item.experienceId || item.id));
  for (const id of parsed.selectedExperienceIds) {
  ...
  for (const id of parsed.selectedProjectIds) {
  ...
  for (const id of parsed.selectedOpinionIds) {
  ...
  for (const id of parsed.selectedWriterInstructionIds) {
    if (!selectedInstructionIds.has(id)) throw Object.assign(new Error("AI_CONTEXT_INVALID"), { code: "AI_CONTEXT_INVALID" });
  }
  const textSet = new Set([parsed.replies.A.text.trim(), parsed.replies.B.text.trim(), parsed.replies.C.text.trim()]);
  if (textSet.size !== 3) throw Object.assign(new Error("AI_CONTEXT_INVALID"), { code: "AI_CONTEXT_INVALID" });
  if (String(candidate.text || "").includes(parsed.replies.A.text) || String(candidate.text || "").includes(parsed.replies.B.text) || String(candidate.text || "").includes(parsed.replies.C.text)) {
    throw Object.assign(new Error("AI_CONTEXT_INVALID"), { code: "AI_CONTEXT_INVALID" });
  }
  for (const reply of Object.values(parsed.replies)) {
...
async function persistReplyDecision(db, admin, candidatePostId, candidate, identity, selected, decision, localScores, usage, regenerationCount, replyDraftId = null, localMock = false) {
  ...
  const draft = {
    ...
    adapterOutput: {
      ...
      selectedContextIds: [...decision.selectedProjectIds, ...decision.selectedExperienceIds, ...decision.selectedOpinionIds],
