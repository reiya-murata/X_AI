...
const { buildLocalScores, validateGeneratedCandidates, calculateRiskPenalty } = require("../src/phase3/analysis");
...
function main() {
  ...
  assert.ok(docs.some((item) => item.collection === "creatorProfiles"));
  assert.ok(docs.some((item) => item.collection === "writerInstructions"));
  ...
  assert.ok(validateGeneratedCandidates(scenario.generation).ok);
  ...
  console.log(JSON.stringify({
    ok: true,
    seedCollections: [...new Set(docs.map((item) => item.collection))],
    candidateRank: rankCandidate(total),
    total,
